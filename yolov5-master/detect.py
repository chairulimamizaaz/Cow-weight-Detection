    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Inisialisasi variabel untuk menyimpan jenis objek saat ini
                obj_counts = {}  
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if obj_class not in obj_counts:
                        obj_counts[obj_class] = 1  
                    else:
                        obj_counts[obj_class] += 1

                    obj_count = obj_counts[obj_class]  
                    ...
                    if save_txt:
                        ...
                        loaded_model = joblib.load("distance_xgb.joblib")
                        new_data = np.array([[x,y,w,h]])
                        distance = loaded_model.predict(new_data)[0]

                        loaded_model = joblib.load("body_xgb.joblib")
                        new_data2 = np.array([[x, y, w, h, distance]])
                        prediction = loaded_model.predict(new_data2)[0]

                        berat = ((prediction[0] ** 2) * prediction[1]) / 10840
